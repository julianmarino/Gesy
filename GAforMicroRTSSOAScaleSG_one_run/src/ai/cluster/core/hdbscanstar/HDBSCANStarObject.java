package ai.cluster.core.hdbscanstar;

import java.io.IOException;
import java.util.ArrayList;

import ai.cluster.core.distance.CosineSimilarity;
import ai.cluster.core.distance.DistanceCalculator;
import ai.cluster.core.distance.EuclideanDistance;
 ...
 * @author zjullion
 */
public class HDBSCANStarObject {
     ...
     * minPoints and minClusterSize. Note that the input file must be a
     * comma-separated value (CSV) file, and that all of the output files will
     * be CSV files as well. The flags "file=", "minPts=", "minClSize=",
     * "constraints=", and "distance_function=" should be used to specify the
     * input data set file, value for minPoints, value for minClusterSize, input
     * constraints file, and the distance function to use, respectively.
     *
     * @param dataSet
     * @param minPoints
     ...
     *
     */
    public static int[] runHDBSCAN(double[][] dataSet, Integer minPoints, Integer minClusterSize, boolean compactHierarchy) throws IOException {
    ...
    /**
     * Parses out the input parameters from the program arguments. Prints out a
     * help message and exits the program if the parameters are incorrect.
     *
    ...
    private static void printHelpMessageAndExit() {
        ...
        System.out.println("The algorithm will produce five files: the hierarchy, cluster tree, final flat partitioning, outlier scores, and an auxiliary file for visualization.");
        System.out.println();
