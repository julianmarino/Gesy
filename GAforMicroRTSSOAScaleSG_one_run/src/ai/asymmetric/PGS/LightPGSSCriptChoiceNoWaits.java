 ...
 * @author rubens
 */
public class LightPGSSCriptChoiceNoWaits extends AIWithComputationBudget implements InterruptibleAI {
    ...
    public LightPGSSCriptChoiceNoWaits(UnitTypeTable utt) {
        this(100, -1, 200, 4, 4,
    ...
    public LightPGSSCriptChoiceNoWaits(UnitTypeTable utt, List<AI> scripts) {
        this(100, -1, 200, 4, 4,
                new SimpleSqrtEvaluationFunction3(),
                //new SimpleSqrtEvaluationFunction2(),
                //new LanchesterEvaluationFunction(),
    ...
    public LightPGSSCriptChoiceNoWaits(UnitTypeTable utt, List<AI> scripts, String tuplaIndSc) {
        this(100, -1, 200, 4, 4,
                new SimpleSqrtEvaluationFunction3(),
                //new SimpleSqrtEvaluationFunction2(),
                //new LanchesterEvaluationFunction(),
    ...
    public LightPGSSCriptChoiceNoWaits(UnitTypeTable utt, List<AI> scripts, int max_playouts, String tuplaIndSc) {
    ...
    public LightPGSSCriptChoiceNoWaits(UnitTypeTable utt, int max_playouts, List<AI> scripts, String tuplaIndSc) {
        this(100, -1, max_playouts, 4, 4,
                new SimpleSqrtEvaluationFunction3(),
                //new SimpleSqrtEvaluationFunction2(),
                //new LanchesterEvaluationFunction(),
    ...
    public LightPGSSCriptChoiceNoWaits(int time, int max_playouts, int la, int a_I, int a_R, EvaluationFunction e, UnitTypeTable a_utt, PathFinding a_pf) {
    ...
    public LightPGSSCriptChoiceNoWaits(int time, int max_playouts, int la, int a_I, int a_R, EvaluationFunction e, UnitTypeTable a_utt, PathFinding a_pf, List<AI> scripts) {
        super(time, max_playouts);
    ...
    * Executa um playout de tamanho igual ao @LOOKAHEAD e retorna o valor
    ...
    @Override
    public AI clone() {
        return new LightPGSSCriptChoiceNoWaits(TIME_BUDGET, ITERATIONS_BUDGET, LOOKAHEAD, I, R, evaluation, utt, pf, scripts);
    }
    ...
    private UnitScriptData doPortfolioSearch(int player, UnitScriptData currentScriptData, AI seedEnemy) throws Exception {
        ...
        while (System.currentTimeMillis() < (start_time + (TIME_BUDGET))) {
            ...
            for (Unit unit : unitsPlayer) {
                ...
                for (AI ai : scripts) {
                	boolean besthasWait=getNameWait(player, bestScriptData, gs_to_start_from, unit);
                    ...
                    boolean currentHasWait=getNameWait(player, currentScriptData, gs_to_start_from, unit);
                    if ((scoreTemp > bestScore && !currentHasWait) || (besthasWait && !currentHasWait) ) {
    ...
    private boolean getNameWait(int player, UnitScriptData uScriptPlayer, GameState gs2, Unit u) {
    	String sAI = uScriptPlayer.getAIUnit(u).toString();
    	UnitAction uAt = getUnitAction(u, cache.get(sAI));
    	if (uAt != null) {
    		if(uAt.getActionName().equals("wait"))
    		{
    			return true;
    		}
    	}
